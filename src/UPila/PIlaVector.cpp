...
namespace UPilaVector
{
    ...
    // importante el resultado solo puede ir desde (-128 <= x <= 255)
    int evalua(char op1, char op2, char sim_operacion)
    {
        switch (sim_operacion)
        {
            ...
            case '*':
                return op1 * op2;
            case '/':
                if(op2 == 0) throw std::runtime_error("Disivion por 0 imposible!!!");
                return op1 / op2;
            case '^':
    ...
    int evaluar_postfija(std::string expPostFija)
    {
        ...
        for (int i = 0; i < expPostFija.length(); i++)
        {
            ...
            else
            {
                ...
                p.sacar(&op2);
                p.sacar(&op1);
                char simbolo_operacion = expPostFija[i];
                int z = evalua(op1, op2, simbolo_operacion);
                p.meter(z);
            }
        }
        return p.cima();
